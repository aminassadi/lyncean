...
{
public:
    static std::tuple<int, std::string, std::vector<std::string>, bool> get_input_parameters(int argc, ARGV& argv);
private:
    static void register_pid(Parser &parser);
    static void register_command(Parser &parser);
    static void register_follow_fork_flag(Parser &parser);
    ...
    static bool extract_follow_fork_flag(Parser& parser);
