...
int tail_raw_syscall_unlink_exit(struct __raw_tracepoint_args *ctx)
{
    ...
    uint32_t cpu = bpf_get_smp_processor_id();
    struct_unlink_syscall *unlink_struct = NULL;
    unlink_struct = bpf_map_lookup_elem(&event_pool, &cpu);
    if (!unlink_struct)
    {
        BPF_PRINTK("ERROR, lookup from unlink_struct_pool failed\n");
        goto out;
    }
    void *ptr_start = (void *)unlink_struct;
    void *ptr_end = (void *)unlink_struct->pathname;
    unlink_struct->syscallid = args->syscallid;
    if (bpf_probe_read(&unlink_struct->rc, sizeof(int), (void *)&PT_REGS_RC((struct pt_regs *)ctx->args[0])) != 0)
    {
        BPF_PRINTK("ERROR, failed to get return code\n");
    }
    long size = bpf_probe_read_str(unlink_struct->pathname, MAX_PATH, (void *)args->arg[0]);
    if (size > 0)
    {
        ptr_end += size;
    }
